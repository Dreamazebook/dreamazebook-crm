...
const AdminCouponNewPage: FC = () => {
  ...
  const [form, setForm] = useState<CouponFormData>({
    ...
    allow_stack_with_promotions: true,
    metadata: null,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { success, data } = await api.post(API_ADMIN_COUPONS, payload);
      if (success && data) {
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center space-x-4">
          <button onClick={() => router.push('/admin/coupons')} className="text-gray-600 hover:text-gray-800">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <h1 className="text-xl font-medium text-gray-900">New Coupon</h1>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateField('created_for', e.target.value)}
                  className={inputClass}
                />
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  value={form.type}
                  onChange={(e) => updateField('type', e.target.value as 'percentage' | 'fixed_amount')}
                  className={inputClass}
                >
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  {form.type === 'percentage' ? 'e.g. 10 = 10% off' : 'Amount in ' + form.currency_code}
                </p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  min={0}
                  value={form.max_uses ?? ''}
                  onChange={(e) => updateField('max_uses', e.target.value ? parseInt(e.target.value) : null)}
                  className={inputClass}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateField('per_user_limit', e.target.value ? parseInt(e.target.value) : null)}
                  className={inputClass}
                  placeholder="Unlimited"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min={0}
                  step={0.01}
                  value={form.min_subtotal_amount ?? ''}
                  onChange={(e) => updateField('min_subtotal_amount', e.target.value ? parseFloat(e.target.value) : null)}
                  className={inputClass}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateField('max_discount_amount', e.target.value ? parseFloat(e.target.value) : null)}
                  className={inputClass}
                  placeholder="No cap"
                />
              </div>
          ...
          <div>
            ...
            <label className="flex items-center space-x-3">
              <input
                type="checkbox"
                checked={form.allow_stack_with_promotions}
                onChange={(e) => updateField('allow_stack_with_promotions', e.target.checked)}
